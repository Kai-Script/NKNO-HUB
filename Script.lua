do
    ...
    safeCall(function()
        ...
        local customTagName = "NKNO$"

        -- === ТАЙМЕР ===
        local targetDate = os.time({year=2026, month=8, day=1, hour=0, min=0, sec=0}) - 3*3600
        ...
        ToggleLabelText.Size = UDim2.new(1, 0, 0.5, 0)
        ToggleLabelText.Position = UDim2.new(0, 0, 0, 2)
        ...
        ToggleLabelText.TextSize = 14
        ToggleLabelText.ZIndex = 2
        ...
        local dragToggle, dragInputT, dragStartT, startPosT, dragStartTime = false, nil, nil, nil, 0
        ...
        local function buildLangButton(emoji, text, posX, langCode)
            ...
            Btn.MouseButton1Click:Connect(function()
                ...
                -- Принудительно открываем GUI после выбора языка
                isMinimized = true
                MainFrame.Visible = false
                ShadowFrame.Visible = false
                MinBtn.Text = "+"
                ...
                print("[nkno$] Интерфейс скрыт, виджет с таймером активен.")
            end)
        end
        ...
        -- Добавляем таймер в раздел миров (в LeftPanel)
        local TimerInWorlds = Instance.new("TextLabel")
        TimerInWorlds.Parent = LeftPanel
        TimerInWorlds.BackgroundTransparency = 1
        TimerInWorlds.Size = UDim2.new(1, 0, 0, 20)
        TimerInWorlds.Position = UDim2.new(0, 0, 0, 0)
        TimerInWorlds.Font = Enum.Font.GothamSemibold
        TimerInWorlds.TextColor3 = Color3.fromRGB(200, 200, 220)
        TimerInWorlds.TextSize = 13
        TimerInWorlds.TextXAlignment = Enum.TextXAlignment.Left
        TimerInWorlds.Text = string.format(L("TimerLabel"), "00:00:00:00")
        -- Обновляем таймер и в этом Label
        local oldTimerFunc = updateTimer
        updateTimer = function()
            oldTimerFunc()
            if timerLabel then
                TimerInWorlds.Text = timerLabel.Text
            end
        end
        ...
        ToggleFrame.Position = UDim2.new(0, 0, 0, 26) -- сместил вниз, чтобы таймер был сверху
        ...
        SliderFrame.Position = UDim2.new(0, 0, 0, 94) -- сместил
        ...
        DistLabel.Position = UDim2.new(0, 0, 0, 174)
        ...
        DropdownBtn.Position = UDim2.new(0, 0, 0, 200)
        ...
        DropdownList.Position = UDim2.new(0, 0, 0, 252)
        ...
        _G.ApplyLanguage = function()
            ...
            local displayText = (currentWorld == "Bbnos World") and "BBNO$ V" or currentWorld
            WorldLabel.Text = string.format(L("WorldLabel"), displayText)
        ...
        local function ensureMenuOpened()
            ...
            if not ToggleWidget.Visible then
                ToggleWidget.Visible = true
            end
        end
        ...
        print("[nkno$] Скрипт загружен! Таймер до 1 августа, TAG, BBNO$ V.")
    end)
end
